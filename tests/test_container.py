...
def test_stream_seek(temp_container, use_compression, use_packing, clean_storage):
    ...
    if clean_storage:
        ...
        assert not os.path.isfile(temp_container._get_loose_path_from_hashkey(hashkey))
...
def test_list_all_objects_extraneous(
    ...
    os.mkdir(os.path.join(temp_container._get_loose_folder(), "INVALID_NAME"))
    ...
    if loose_prefix_len != 0:
        ...
        invalid_first_level = os.path.join(temp_container._get_loose_folder(), prefix)
        ...
        first_level_subfolder = os.path.join(
            temp_container._get_loose_folder(), "0" * loose_prefix_len
        )
        try:
...
def test_delete(
    ...
    temp_container.add_object(obj1)
    assert os.path.exists(temp_container._get_loose_path_from_hashkey(hashkey1))
    ...
    assert not os.path.exists(temp_container._get_loose_path_from_hashkey(hashkey1))
...
def test_delete_with_duplicates(temp_container):
    ...
    os.chmod(
        os.path.dirname(temp_container._get_loose_path_from_hashkey(hashkey)),
        stat.S_IREAD | stat.S_IEXEC,
    )
    os.chmod(
        os.path.dirname(temp_container._get_loose_path_from_hashkey(hashkey2)),
        stat.S_IREAD | stat.S_IEXEC,
    )
    os.chmod(
        temp_container._get_loose_path_from_hashkey(hashkey),
...
def test_clean_storage_with_duplicates(
    ...
    # to change the permissions on the file, instead.
    os.chmod(
        os.path.dirname(temp_container._get_loose_path_from_hashkey(hashkey1)),
        stat.S_IREAD | stat.S_IEXEC,
    )
    os.chmod(
        os.path.dirname(temp_container._get_loose_path_from_hashkey(hashkey2)),
    ...
    os.chmod(
        os.path.dirname(temp_container._get_loose_path_from_hashkey(hashkey3)),
        stat.S_IREAD | stat.S_IEXEC,
    )
    os.chmod(
        temp_container._get_loose_path_from_hashkey(hashkey1),
...
def test_lazy_loose_stream(temp_container):
    ...
    assert not os.path.exists(loosepath)
    ...
    assert os.path.exists(loosepath)
...
def test_lazy_loose_loosen_and_delete(
    ...
    def new_loosen_object(hashkey):
        ...
        assert os.path.exists(self._get_loose_path_from_hashkey(hashkey))
        ...
        if self.call_counter < num_iterations_delete:
            self.clean_storage()
            assert not os.path.exists(self._get_loose_path_from_hashkey(hashkey))
