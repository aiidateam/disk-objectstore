...
def test_clean_loose_per_pack(temp_container, generate_random_data):
    ...
    assert min_num_packs <= actual_packs <= max_num_packs, (
    ...
    if len(unique_loose_counts) >= 2:
        ...
        assert avg_decrease >= min_decrease, (
    ...
    assert loose_when_packing_starts < num_objects, (
    ...
    assert objects_cleaned_during_packing >= num_objects_per_pack, (
...
def test_clean_loose_per_pack_with_open_file(temp_container, generate_random_data):
    ...
    if platform.system() == 'Windows':
        ...
        assert not loose_path.exists() and final_counts['loose'] == 0, (
