...
if __name__ == "__main__":
    ...
    # Read
    start = time.time()
    to_compare = set((obj.uuid, obj.compressed) for obj in session_uuid.query(ObjUUID).filter(ObjUUID.uuid.in_(uuids)))
    tot_time = time.time() - start
    ...
    for sublist in chunk(uuid_uint64s, 500):
        conditions = (and_(ObjInt.uuidint1==x, ObjInt.uuidint2==y) for (x, y) in sublist)

        #    ## This works with PostgreSQL, but breaks with SQLite
        #    #.filter(tuple_(ObjInt.uuidint1, ObjInt.uuidint2).in_(uuid_uint64s)

        to_compare = [
            (hex_from_ints(int_from_bytes(obj.uuidint1), int_from_bytes(obj.uuidint2)), obj.compressed)
            for obj in 
            session_int.query(ObjInt).filter(or_(*conditions))]
        results += to_compare
    ...
    for idx, (before, after) in enumerate(zip(sorted(uuids), sorted(results))):
        assert (before, False) == after, "First mismatch at pos {}/{}: {} vs {}".format(
